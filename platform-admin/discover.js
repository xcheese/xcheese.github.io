(function () {
  ...
  const extraShops = [
    { id: "S-14001", name: "FrameByFrame", owner: "阿帧", emoji: "🎬", creatorCategory: "视频创作者", favorites: 21840, members: 4860, conversion: 6.4, revenueChange: 32.8, value: "高价值", desc: "幕后拆解、镜头语言课程和每月短片制作日志。" },
    { id: "S-14002", name: "TinyForge Games", owner: "小锤", emoji: "🎮", creatorCategory: "独立游戏", favorites: 18420, members: 3620, conversion: 8.1, revenueChange: 48.2, value: "高潜力", desc: "开发像素叙事游戏，分享试玩版、原声与开发周报。" },
    { id: "S-14003", name: "OpenKit Lab", owner: "林开", emoji: "💻", creatorCategory: "软件开发", favorites: 12680, members: 2180, conversion: 5.6, revenueChange: 24.4, value: "增长", desc: "开源效率工具、自动化脚本和独立软件开发记录。" },
    { id: "S-14004", name: "Panel Planet", owner: "栗子", emoji: "🗯️", creatorCategory: "漫画家", favorites: 24860, members: 5280, conversion: 7.2, revenueChange: 18.6, value: "高价值", desc: "连载科幻漫画、角色设定集和读者限定番外。" },
    { id: "S-14005", name: "ModMosaic", owner: "摩德", emoji: "🧩", creatorCategory: "游戏模组", favorites: 16840, members: 2940, conversion: 6.8, revenueChange: 42.1, value: "高潜力", desc: "制作模拟人生4、Minecraft 模组与场景资源。" },
    { id: "S-14006", name: "Pattern Forest", owner: "森织", emoji: "🧶", creatorCategory: "图纸与教程", favorites: 9860, members: 1640, conversion: 4.9, revenueChange: 16.3, value: "成长", desc: "毛衣编织、3D 打印和手作图纸，提供分步视频教程。" }
  ];
  ...
  function favoriteButton(item) {
    return `<button class="favorite-button ${favorite(item.id) ? "active" : ""}" data-favorite="${item.id}" aria-label="${favorite(item.id) ? "取消收藏" : "收藏"}">${favorite(item.id) ? "💜" : "🤍"}</button>`;
  }
  ...
  function render() {
    ...
    root.innerHTML = `
      <header class="market-header"><div class="header-inner"><a class="market-brand" href="./discover.html"><i>🌟</i><span>发现创作者</span></a><nav class="market-nav"><a href="#shops">店铺</a><a href="#memberships">会员</a><a href="#crowdfunding">众筹</a><a href="#products">商品</a></nav><label class="market-search">🔍<input id="market-search" type="search" value="${esc(state.query)}" placeholder="搜索创作者、众筹、商品…"></label><a class="header-action" href="./index.html">📌 平台后台</a></div></header>
      <main class="market-main">
        ...
        <section class="personalize"><div class="personalize-head"><strong>选择你感兴趣的创作</strong><span>已选 ${state.interests.length} 项 · 保存在当前浏览器</span></div><div class="interest-list">${interests.map((interest) => `<button class="interest-chip ${state.interests.includes(interest) ? "active" : ""}" data-interest="${interest}">${interestEmoji(interest)} ${interest}</button>`).join("")}</div></section>
  ...
  function interestEmoji(interest) {
    return ({ 全部: "✨", 视频: "🎬", 独立游戏: "🎮", 软件: "💻", 音乐: "🎵", 漫画: "📖", 插画: "🎨", 手作: "🔨", 游戏模组: "🧩", "3D打印": "🏰", 编织图纸: "🧶", Minecraft: "⛏️" })[interest] || "💜";
  }

  function shopCard(shop) {
    return `<article class="shop-card" data-open="shop" data-id="${shop.id}"><div class="shop-top"><div class="creator-avatar">${shop.emoji || "🎨"}</div><div class="shop-name"><h3>${esc(shop.name)}</h3><span>${esc(shop.creatorCategory || shop.category)} · ${esc(shop.owner)}</span></div>${favoriteButton(shop)}</div><p>${esc(shop.desc || "持续分享创作内容、作品与会员限定更新。")}</p><div class="shop-stats"><span>收藏<strong>${Number(shop.favorites || 0).toLocaleString()}</strong></span><span>会员<strong>${Number(shop.members || 0).toLocaleString()}</strong></span><span>近期收入<strong>${shop.revenueChange >= 0 ? "+" : ""}${shop.revenueChange || 0}%</strong></span></div></article>`;
  }

  function membershipCard(plan) {
    return `<article class="membership-card" data-open="membership" data-id="${plan.id}"><span>${plan.status === "销售中" ? "💜" : "⏸️"} ${esc(plan.shop)}</span><h3>${esc(plan.name)}</h3><p>${plan.active.toLocaleString()} 位有效会员 · 续费率 ${plan.renewal}% · ${plan.cycle}</p><div class="membership-price"><strong>¥${plan.price}</strong><span>每 ${plan.cycle}</span></div></article>`;
  }

  function crowdCard(item) {
    return `<article class="crowd-card" data-open="crowdfunding" data-id="${item.id}"><div class="crowd-visual">${item.emoji}</div><h3>${esc(item.name)}</h3><p>${esc(item.shop)} · ${esc(item.category)} · 剩余 ${item.daysLeft} 天</p><div class="progress"><i style="width:${Math.min(100, item.progress)}%"></i></div><div class="crowd-stats"><span><strong>${item.progress}%</strong> 达成</span><span>💜 ${item.favorites.toLocaleString()}</span><span>${item.supporters.toLocaleString()} 人支持</span>${favoriteButton(item)}</div></article>`;
  }

  function productCard(item, index) {
    return `<article class="product-card" data-open="product" data-id="${item.id}"><div class="product-emoji">${productEmoji[D.products.indexOf(item)] || productEmoji[index % productEmoji.length]}</div><div class="product-copy"><h3>${esc(item.name)}</h3><p>${esc(item.shop)} · ${esc(item.type)} · 💜 ${item.favorites.toLocaleString()}</p><strong>¥${item.price}</strong></div>${favoriteButton(item)}</article>`;
  }
  ...
  function showDetail(kind, id) {
    ...
    const emoji = item.emoji || (kind === "membership" ? "💜" : kind === "product" ? productEmoji[D.products.indexOf(item)] : "🌟");
