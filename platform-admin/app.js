(function () {
  ...
  function shell() {
    ...
    return `<div class="app-shell">
      <aside class="sidebar" aria-label="主导航">
        ...
        <nav class="nav-list">
          ${D.nav.map((item) => `<a class="nav-item ${route.name === item.id ? "active" : ""}" href="#/${item.id}" data-nav="${item.id}">${icon(item.icon)}<span>${item.label}</span>${["shops", "memberships", "products", "users", "orders", "tickets", "traffic", "reports", "settings"].includes(item.id) ? icon("down", "chevron") : ""}</a>`).join("")}
        </nav>
  ...
  function renderPage(route) {
    if (route.id && ["shops", "memberships", "products", "users", "orders", "finance", "tickets", "traffic"].includes(route.name)) {
  ...
  const configs = {
    shops: {
      title: "店铺", desc: "识别高价值店铺、经营异常与增长机会", data: () => D.shops, search: "搜索店铺名称、店主或负责人",
      summary: [["店铺总数", "12,648", "↑ 8.4%"], ["活跃店铺", "8,614", "↑ 9.8%"], ["高价值店铺", "1,286", "↑ 12.6%"], ["风险店铺", "184", "↓ 6.2%"]],
      columns: [["name", "店铺", "primary"], ["value", "客户价值", "status"], ["gmv", "GMV", "money"], ["net", "净收入", "money"], ["members", "付费会员", "integer"], ["conversion", "转化率", "percent"], ["refund", "退款率", "percent"], ["complaints", "客诉", "integer"], ["risk", "风险", "status"], ["manager", "负责人", "text"], ["note", "最近备注", "text"]]
    },
    memberships: {
    ...
    products: {
      title: "商品", desc: "统一管理数字商品、虚拟服务和实体商品履约表现", data: () => D.products, search: "搜索商品、店铺或商品编号",
      summary: [["在售商品", "36,284", "↑ 11.3%"], ["数字商品占比", "61.8%", "↑ 3.1pp"], ["实体待发货", "312", "需处理"], ["商品净收入", "¥12,840,620", "↑ 16.9%"]],
      columns: [["name", "商品", "primary"], ["shop", "所属店铺", "text"], ["type", "类型", "status"], ["price", "售价", "money"], ["sales", "销量", "integer"], ["revenue", "净收入", "money"], ["conversion", "购买转化", "percent"], ["refund", "退款率", "percent"], ["stock", "库存/额度", "text"], ["status", "状态", "status"]]
    },
    users: {
      title: "用户", desc: "按消费价值、复购倾向与服务风险管理平台客户", data: () => D.users, search: "搜索用户昵称或用户编号",
      summary: [["注册用户", "2,846,920", "↑ 12.4%"], ["月活用户", "684,230", "↑ 8.9%"], ["付费用户", "316,842", "↑ 14.7%"], ["高价值客户", "28,614", "↑ 17.2%"]],
      columns: [["name", "用户", "primary"], ["segment", "客户分层", "status"], ["spend", "累计消费", "money"], ["orders", "订单数", "integer"], ["memberships", "会员数", "integer"], ["shops", "消费店铺", "integer"], ["refund", "退款率", "percent"], ["complaint", "客诉", "integer"], ["lastActive", "最近活跃", "text"], ["status", "状态", "status"]]
    },
    orders: {
  ...
  function shopTable(data, compact) {
    const columns = configs.shops.columns.filter((column) => compact ? !["status", "joined", "region"].includes(column[0]) : true);
  ...
  function renderTraffic() {
    const columns = [["title", "页面", "primary"], ["path", "路径", "text"], ["type", "页面类型", "status"], ["shop", "所属店铺", "text"], ["uv", "访问人数", "integer"], ["pv", "访问次数", "integer"], ["duration", "平均访问时长", "text"], ["bounce", "跳出率", "percent"], ["conversion", "购买转化", "percent"], ["revenue", "归因收入", "money"]];
    const data = filteredData(D.pages, columns);
    return `${pageHeading("流量与页面", "从注册、访问、加购到支付分析完整转化链路", `<button class="control" data-action="export" data-export="pages">${icon("download")}导出明细</button><button class="primary-button" data-action="create-report">创建分析</button>`)}
      ${summaryStrip([["访问人数", "1,286,420", "↑ 16.8%"], ["平均访问时长", "02:16", "↑ 12秒"], ["注册转化率", "8.42%", "↑ 0.76pp"], ["购买转化率", "3.62%", "↑ 0.42pp"]])}
      <div class="dashboard-grid"><section class="panel"><div class="panel-head"><div class="panel-title">访问与购买趋势</div><div class="segmented"><button class="active">访问</button><button>注册</button><button>购买</button></div></div><div class="chart-wrap">${lineChart(D.dashboard.trend.map((v, i) => v * (5.2 + (i % 4) * .12)), "流量")}</div></section><section class="panel"><div class="panel-head"><div class="panel-title">转化漏斗</div><span class="panel-meta">按访问用户去重</span></div><div class="insight-list">${[["访问", "1,286,420", "100%"], ["注册", "108,324", "8.42%"], ["查看商品/方案", "492,680", "38.30%"], ["发起支付", "61,274", "4.76%"], ["支付成功", "46,566", "3.62%"]].map((item, i) => `<div class="insight-item"><span class="insight-index">${i + 1}</span><span><strong>${item[0]}</strong><small>${item[1]} 人</small></span><span class="value-tag">${item[2]}</span></div>`).join("")}</div></section></div>
      <section class="panel">${toolbar("搜索页面标题、路径或店铺", D.pages)}${dataTable(data, columns, "traffic")}${pagination(data.length, D.pages.length)}</section>`;
  }
  ...
  function entitySource(type) {
    return ({ shops: D.shops, memberships: D.memberships, products: D.products, users: D.users, orders: D.orders, finance: D.finance, tickets: D.tickets, traffic: D.pages })[type] || [];
  }
  ...
  function renderDetail(type, id) {
    ...
    const tabs = [["overview", "概览"], ["analysis", "数据分析"], ["activity", type === "orders" ? "订单链路" : type === "tickets" ? "处理记录" : "关联记录"], ["notes", "标注与备注"]];
    return `<section class="panel detail-hero"><div class="detail-top"><div class="detail-avatar">${esc(String(title).slice(0, 1))}</div><div class="detail-title"><h1>${esc(title)}</h1><p>${esc(subtitle)}</p></div><div class="detail-actions"><a class="control" href="#/${type}">${icon("arrowLeft")}返回列表</a><button class="ghost-button" data-action="add-note" data-target="${esc(title)}">${icon("note")}添加备注</button><button class="primary-button" data-action="detail-primary" data-type="${type}">${detailActionLabel(type)}</button></div></div><div class="detail-facts">${facts.map((fact) => `<div class="detail-fact"><label>${fact[0]}</label><strong>${fact[1]}</strong></div>`).join("")}</div></section>
      <section class="panel"><div class="tabs">${tabs.map((tab) => `<button class="tab ${state.detailTab === tab[0] ? "active" : ""}" data-detail-tab="${tab[0]}">${tab[1]}</button>`).join("")}</div><div class="tab-content">${detailTabContent(type, item, title)}</div></section>`;
  }

  function detailFacts(type, item) {
    const maps = {
      shops: [["客户分层", status(item.value)], ["近30天 GMV", money.format(item.gmv)], ["净收入", money.format(item.net)], ["付费会员", integer.format(item.members)], ["购买转化", `${item.conversion}%`], ["退款率", `${item.refund}%`]],
      ...
      products: [["状态", status(item.status)], ["类型", item.type], ["售价", money.format(item.price)], ["销量", integer.format(item.sales)], ["购买转化", `${item.conversion}%`], ["退款率", `${item.refund}%`]],
      users: [["客户分层", status(item.segment)], ["累计消费", money.format(item.spend)], ["订单数", item.orders], ["会员数", item.memberships], ["退款率", `${item.refund}%`], ["最近活跃", item.lastActive]],
  ...
  function detailActionLabel(type) {
    return ({ shops: "调整客户分层", memberships: "查看到期会员", products: "检查商品", users: "创建运营任务", orders: "处理订单", finance: "复核申请", tickets: "更新工单", traffic: "创建页面分析" })[type] || "处理";
  }
  ...
  function fieldLabel(key) {
    return ({ id: "编号", owner: "店主/负责人", value: "客户分层", gmv: "GMV", net: "净收入", members: "会员数", conversion: "转化率", refund: "退款率", complaints: "客诉数", risk: "风险", manager: "平台负责人", status: "状态", joined: "加入时间", category: "类目", region: "地区", note: "最近备注", shop: "所属店铺", price: "价格", cycle: "有效期", active: "有效会员", newCount: "新增会员", renewal: "续费率", revenue: "收入", churn: "流失率", type: "类型", sales: "销量", stock: "库存", segment: "客户分层", spend: "累计消费", orders: "订单数", memberships: "会员数", lastActive: "最近活跃", time: "时间", user: "用户", item: "商品/方案", amount: "金额", paid: "实付", payment: "支付方式", kind: "类型", related: "关联记录", fee: "手续费", reason: "原因", requested: "发起时间", channel: "渠道", priority: "优先级", age: "已等待", sla: "SLA", path: "页面路径", uv: "访问人数", pv: "访问次数", duration: "平均时长", bounce: "跳出率" })[key] || key;
  }

  function detailInsights(type) {
    return ({ shops: [["收入贡献", "平台同类店铺前 8%", "高价值"], ["会员健康", "续费率稳定但新增放缓", "需增长"], ["风险提示", "近7天流量出现异常波动", "P1"]], memberships: [["续费表现", "高于同周期方案均值", "+6.2pp"], ["到期风险", "7天内到期会员占比", "12.4%"], ["价格机会", "相邻价位转化仍有空间", "可测试"]], products: [["转化表现", "高于同类型商品均值", "+2.8pp"], ["退款原因", "主要集中在描述预期", "需优化"], ["流量机会", "搜索来源增长明显", "+18.4%"]], users: [["客户价值", "近12个月消费排名", "Top 3%"], ["复购倾向", "跨店消费意愿较高", "76%"], ["服务风险", "近期客诉与退款行为", "低风险"]], orders: [["支付链路", "支付回调与到账一致", "正常"], ["履约状态", "当前节点未超平台时限", "正常"], ["风险校验", "设备与支付账户一致", "低风险"]], finance: [["金额影响", "占店铺本期结算比例", "3.8%"], ["历史表现", "近90天同类申请通过率", "84%"], ["审核建议", "核对材料后人工复核", "需处理"]], tickets: [["客户价值", "用户属于稳定复购客群", "较高"], ["问题集中度", "同店铺近7天相似工单", "12 件"], ["处理建议", "优先给出明确履约时间", "P0"]], traffic: [["流量质量", "访问时长高于同类页面", "+18秒"], ["转化表现", "购买转化高于平台均值", "+1.2pp"], ["来源机会", "站内推荐贡献增长", "+16.4%"]] })[type] || [];
  }
  ...
  function globalSearch(query) {
    ...
    if (!q) return;
    const collections = [["店铺", "shops", D.shops], ["会员方案", "memberships", D.memberships], ["商品", "products", D.products], ["用户", "users", D.users], ["订单", "orders", D.orders], ["客诉", "tickets", D.tickets]];
  ...
  function exportCsv(type) {
    const source = ({ shops: D.shops, memberships: D.memberships, products: D.products, users: D.users, orders: D.orders, finance: D.finance, pages: D.pages })[type] || [];
